...
def _load_builtin_templates() -> dict[str, dict[str, Any]]:
    """Load all JSON template files from the templates/ subdirectory."""
        ...
        for template_file in sorted(_TEMPLATES_DIR.glob("*.json")):
            try:
                ...
                if template_id:
                    ...
                    logger.warning("Template file %s has no 'template_id', skipping", template_file.name)
            except Exception as exc:
                logger.warning("Failed to load template file %s: %s", template_file.name, exc)
    return templates
